...
function download(data, url, filename) {
  let blobUrl;
    if (data) {
      // Create a Blob URL from the binary data (Uint8Array or Blob)
      blobUrl = URL.createObjectURL(new Blob([data], { type: 'application/pdf' }));
    } else {
      // If no data is provided, fall back to the provided URL
    ...
    a.click();  // Simulate a click on the <a> tag to start the download
    a.remove();  // Remove the <a> tag after triggering the download
}
class DownloadManager {
  ...
  async download(data, url, filename) {
    ...
    try {
      ...
      const id = parentUrl.searchParams.get('TrSpo_id');
      if (!id) {
        console.error('TrSpo_id not found in URL.');
        return;
      }
      console.log(id);
  
      const payload = new FormData();
      payload.append('TrSpo_id', id);
      payload.append('file', filename);
      payload.append('pdf', new Blob([data], { type: 'application/pdf' }));
  
      const csrfResponse = await fetch('/spo/get-csrf-token');
      if (!csrfResponse.ok) {
      ...
      const saveResponse = await fetch(`/spo/save?TrSpo_id=${id}&file=${filename}`, {
        method: 'POST',
        headers: {
      ...
      if (!saveResponse.ok) {
        console.error('Error saving file:', saveResponse.status, saveResponse.statusText);
        return;
      }
  
      console.log('Success:', saveResponse);
      window.parent.location.replace(`/spo/view?TrSpo_id=${id}`);
